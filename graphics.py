...
# Modified by Rory McGurty
# Add buttons lsit for window. Added state parameter for Button
# Added GraphWin.clear() method to clear canvas
# Added Button class
# GraphWin._mouseCallback is now the mouse number
# Added _onMotion event to GraphWin
...
class GraphWin(tk.Canvas):
    ...
    def __init__(self, title="Graphics Window",
        ...
        self.items = []
        self.buttons = []                         #My addition
        self.mouseX = None
        self.mouseY = None
        self.bind("<Button-1>", self._onClick)
        self.bind("<B1-Motion>", self._onMotion)
        self.bind("<Button-3>", self._onClick)    #My addition
...
# My addition
class Button:
    def __init__(self, text, width, height, bd, command, state = 'active'):
        self.text = text
        self.width = width
        self.height = height
        self.bd = bd
    ...
    def __repr__(self):
        return "Button( " + self.text + ", " + self.command.__str__() + ")"
    
    def draw(self, graphwin, x, y):
        btn = tk.Button(graphwin, text=self.text, width=self.width,
             height=self.height, bd=self.bd, command=self.command, state = self.state)
        self.x = x
        self.y = y
        btn.place(x = self.x, y = self.y)
        graphwin.addButton(self)
